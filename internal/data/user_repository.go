...
func (ur UserRepository) GetAll(ctx context.Context) ([]user.User, error) {
	...
	q := `SELECT id, first_name, last_name, username, email, picture,
	...
	for rows.Next() {
		var u user.User
		rows.Scan(&u.ID, &u.FirstName, &u.LastName, &u.Username, &u.Email, &u.Picture, &u.CreatedAt, &u.UpdatedAt)
...
func (ur UserRepository) GetByUsername(ctx context.Context, username string) (user.User, error) {
	q := `SELECT id, first_name, last_name, username, email, picture, password,
        created_at, updated_at
        FROM users
		WHERE username=$1; 
		`
...
func (ur UserRepository) Create(ctx context.Context, user *user.User) error {
	...
	row := ur.Data.DB.QueryRowContext(ctx, q, user.FirstName, user.LastName, user.Username, user.Email,
		user.Picture, user.PasswordHash, time.Now(), time.Now())
...
func (ur UserRepository) Update(ctx context.Context, id uint, user user.User) error {
	q := `
	...
	_, err = stmt.ExecContext(
		ctx, user.FirstName, user.LastName, user.Email,
		user.Picture, time.Now(), id,
	)
	if err != nil {
		return err
	}
